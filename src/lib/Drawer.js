// Thanks to https://github.com/stoeffel/react-motion-drawer
...
export default class Drawer extends React.Component {
  ...
  onPan (e) {
    if (this.isClosed() && this.props.noTouchOpen) return false
    if (this.isOpen() && this.props.noTouchClose) return false
    e.preventDefault()

    const { isFinal, pointers, direction, deltaX } = e
    if (this.inPanTolerance(deltaX)) return false
    ...
    const { currentState } = this.state
    const { right, peakingWidth, width } = this.props
